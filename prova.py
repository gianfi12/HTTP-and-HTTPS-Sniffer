...
def pulizia_memoria():
    while True:
        ...
        try:
            ...
            tempo2=-1
        while int(float(tempo2))+100<(time.time()-start_time) and tempo2!=-1:
            name_org[j*6:j*6+6]=[]
            j=j+1
            try:
        ...
        try:
            ...
            tempo3=-1
        while int(float(tempo3))+15<(time.time()-start_time) and tempo3!=-1:
            j=j+1
            retrasmission[j*7:j*7+7]=[]
            try:
                tempo3=name_org[j*7+6]
            except:
                tempo3=-1
...
def control():
    while True:
        print 'CPU:',psutil.cpu_percent(interval=1),'MEMORY:', psutil.phymem_usage().percent,(time.time()-start_time)
        print
        time.sleep(10)
...
p_m=threading.Thread(target=pulizia_memoria)
p_m.start()
print p_m.isAlive()
p1.start()
print p1.isAlive()
