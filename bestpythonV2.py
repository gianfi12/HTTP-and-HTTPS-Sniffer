...
def analize_packet(pkt,dp,sp,flag,lenght,connection):
    ips=pkt[0]
    ipd=pkt[1]
    if flag.find('R',0,len(flag))!=-1:
        var=0
        puntatore=0
        while var<len(connection) and puntatore!=-1:
            try:
            ...
            if puntatore!=-1 and connection[puntatore+1]==ipd and connection[puntatore+2]==sp and connection[puntatore+3]==dp:
                var=len(connection)
                connection[puntatore+8]=connection[puntatore+8]+lenght
                while True:
                    try:
                        ...
                        pass
                f.write(str(connection[puntatore:puntatore+10]).strip('[]\'')+' '+'R'+'\n')
                fcntl.flock(f, fcntl.LOCK_UN)
                f.close()
                #print str(connection[puntatore:puntatore+10]).strip('[]\''),'R',flag
                connection[puntatore:puntatore+10]=[]
            elif puntatore!=-1 and connection[puntatore-1]==ipd and connection[puntatore+1]==dp and connection[puntatore+2]==sp:
                var=len(connection)
                connection[puntatore+7]=connection[puntatore+7]+lenght
                while True:
                    try:
                        f=open('/nf/access.log','a+')
                        fcntl.flock(f, fcntl.LOCK_EX | fcntl.LOCK_NB)
                        ...
                        pass
                f.write(str(connection[puntatore-1:puntatore+9]).strip('[]\'')+' '+'R'+'\n')
                fcntl.flock(f, fcntl.LOCK_UN)
                f.close()
                #print str(connection[puntatore-1:puntatore+9]).strip('[]\''),'R',flag
                connection[puntatore-1:puntatore+9]=[]
            elif  puntatore!=-1:
                var=var+9
            else:
                var=len(connection)
                ...
                if connection[puntatore+6]==3:
                    ...
                    while True:
                        try:
                            ...
                            pass
                    f.write(str(connection[puntatore:puntatore+10]).strip('[]\'')+' '+'F'+'\n')
                    fcntl.flock(f, fcntl.LOCK_UN)
                    f.close()
                    #print str(connection[puntatore:puntatore+10]).strip('[]\''),'F',flag
                    connection[puntatore:puntatore+10]=[]
                else:
                    connection[puntatore+6]=2
            elif puntatore!=-1 and connection[puntatore-1]==ipd and connection[puntatore+1]==dp and connection[puntatore+2]==sp:
                var=len(connection)
                if connection[puntatore+5]==2:
                    connection[puntatore+7]=connection[puntatore+7]+lenght
                    while True:
                        try:
                            f=open('/nf/access.log','a+')
                            ...
                            pass
                    f.write(str(connection[puntatore-1:puntatore+9]).strip('[]\'')+' '+'F'+'\n')
                    fcntl.flock(f, fcntl.LOCK_UN)
                    f.close()
                    #print str(connection[puntatore-1:puntatore+9]).strip('[]\''),'F',flag
                    connection[puntatore-1:puntatore+9]=[]
                else:
                    connection[puntatore+5]=3
            elif puntatore!=-1:
                var=var+9
            else:
                var=len(connection)
        ...
        k=0
        puntatore=0
        while k<len(connection) and puntatore!=-1:
            try:
                puntatore=connection.index(ips,k,len(connection))
            except:
                puntatore=-1
        ...
        while k<len(connection) and puntatore!=-1:
            try:
                puntatore=connection.index(ipd,k,len(connection))
            except:
                puntatore=-1
            if puntatore!=-1 and connection[puntatore-1]==ips and connection[puntatore+1]==sp and connection[puntatore+2]==dp:
                connection[puntatore+7]=int(connection[puntatore+7])+lenght
                k=len(connection)
            else:
                k=k+8
        if k==len(connection) or puntatore==-1:
            connection.extend([ips,ipd,sp,dp,'0','0',0,0,lenght,str(time.time()-start_time)])
    else:
        var=0
        while var<len(connection):
            ...
            if puntatore_1!=-1 and connection[puntatore_1+1]==ipd and connection[puntatore_1+2]==sp and connection[puntatore_1+3]==dp:
                connection[puntatore_1+8]=connection[puntatore_1+8]+lenght
                connection[puntatore_1+9]=str(time.time()-start_time)
                var=len(connection)
                ...
                connection[puntatore_1+8]=str(time.time()-start_time)
                var=len(connection)
            else:
                var=var+9
        ...
        if proto=='06':
                ...
                while index_c<len(stringa) and index!=-1:
                    ...
                    if len(stringa)-index_c<lenght_TLS_c and type=='0b':
                        ...
                        try:
                            ...
                            l=0
                            try:
                                index_cert_org=connection.index(ipd,0,len(connection))
                            except:
                                index_cert_org=-1
                            while l<len(connection) and index_cert_org!=-1:
                                if connection[index_cert_org+1]==ips and connection[index_cert_org+2]==dp and connection[index_cert_org+3==sp]:
                                    l=len(connection)
                                    connection[index_cert_org+4]=name
                                    connection[index_cert_org+9]=time.time()-start_time
                                else:
                                    l=index_cert_org+9
                                if l<len(connection):
                                    try:
                                        index_cert_org=connection.index(ipd,l,len(connection))
                                    except:
                                        index_cert_org=-1
                        except:
                            print certificate
            ...
            if dp==443:
                ...
                if e_name!='0':
                    l=0
                    try:
                        index_e_name=connection.index(ips,0,len(connection))
                    except:
                        index_e_name=-1
                    while l<len(connection) and index_e_name!=-1:
                        if connection[index_e_name+1]==ipd and connection[index_e_name+2]==sp and connection[index_e_name+3]==dp:
                            connection[index_e_name+5]=e_name
                            connection[index_e_name+9]=time.time()-start_time
                            l=len(connection)
                        else:
                            l=index_e_name+9
                        if l<len(connection):
                            try:
                                index_e_name=connection.index(ips,l,len(connection))
                            except:
                                index_e_name=-1
            if sp==443 or dp==443:
                flag=obtain_flag(p)
                analize_packet(pkt,dp,sp,flag,lenght_T,connection)
